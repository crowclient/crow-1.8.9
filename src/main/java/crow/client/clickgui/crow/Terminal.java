...
public class Terminal {
    ...
    public void draw() {
        ...
        crow.client.utils.RenderUtils.drawRoundedRect(x, y, x + width, y + (opened ? height : barHeight), 6, 0xff2D3742);

        if (opened) {
            crow.client.utils.RenderUtils.drawRoundedRect(x + border, y + barHeight, (x + width) - border, (y + height) - border, 6, new Color(51, 51, 51, 210).getRGB());

            crow.client.utils.RenderUtils.drawRoundedRect((x + width) - resizeButtonSize, (y + height) - resizeButtonSize, x + width, y + height, 6, new Color(79, 104, 158).getRGB());
        }
    ...
    private void proccessInput() {
        if (!inputText.isEmpty())
            try {
                String[] tokens = parseTokens(inputText);
                if (tokens.length == 0) return;
                String command = tokens[0];
                String[] args = new String[tokens.length - 1];
                System.arraycopy(tokens, 1, args, 0, args.length);
                Crow.commandManager.executeCommand(command, args);
