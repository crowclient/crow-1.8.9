...
public class SpaciousCategoryTab {
    ...
    /** Backdrop-blur radius for a column's own frosted glass. Matches
     *  {@link RenderUtils#drawFrostedPanel}'s default so panels frost the same
     *  everywhere. */
    private static final int BACKDROP_BLUR_RADIUS = 8;
    ...
    /** Panel fill opacity. The palette ships these at 0xD0, which is already
     *  translucent, but every row painted its own 0xD0 card on top and two
     *  stacked layers land at ~97% — opaque in everything but name. Rows are
     *  transparent now (see {@link SpaciousModuleCard}), so this is the only
     *  layer between the blurred world and the content. */
    static final int GLASS_BODY_ALPHA = 0xA6;
    static final int GLASS_HEADER_ALPHA = 0xB8;
    ...
    /**
     * Frosted backdrop for this column, painted straight onto the world before
     * the AA pass opens. Only the area a column covers gets blurred, so the
     * rest of the screen stays sharp.
     *
     * <p>Reads animation state but never advances it — {@link #draw} owns the
     * {@code update()} calls, and stepping them twice a frame would double
     * every animation's speed.
     */
    void drawBackdropBlur(int mouseX, int mouseY) {
        float open = openAnim.get();
        if (open < 0.01F) return;

        // Same drag follow as draw(), which runs later in the same frame.
        // Idempotent; without it the frost lags the panel by one frame.
        if (dragging) {
        ...
        int blurH = (collapseAnim.get() > 0.005F ? totalH : HEADER_HEIGHT);
        if (blurH <= 0) return;

        // Mirror draw()'s zoom, or the frost sits at full size behind a
        // half-scaled column while the GUI is opening.
        ...
        // ponytail: one capture + two blur passes per column, ~9 columns a
        // frame. Fine for a click GUI; if it ever shows in a frame profile,
        // blur the grid's union bounds once instead of per column.
        GUIBlurUtil.drawBlurredBackground(x, y, TAB_WIDTH, blurH,
                BACKDROP_BLUR_RADIUS, CARD_RADIUS, open);
    ...
    public void draw(int mouseX, int mouseY, CompactPalette palette) {
        ...
        //
        // Both fills run well below opaque so the frosted backdrop reads
        // through them — that blur is already on the framebuffer from this
        // column's own drawBackdropBlur pass, painted before the AA pass.
        // They're drawn as two NON-OVERLAPPING rects: painting the header over
        // a full-height background composites two translucent layers into one
        // near-opaque one, which is why the glass never showed.
        int headerFill = withAlpha(
                CompactModuleCard.blendColor(palette.background, palette.hoverCard, 0.55F),
                (int) (GLASS_HEADER_ALPHA * openAnim.get()));
        int bodyFill = withAlpha(palette.background, (int) (GLASS_BODY_ALPHA * openAnim.get()));
        if (drawBody) {
            RenderUtils.drawRoundedRectAA(x, y, x + TAB_WIDTH, bodyTop,
                    CARD_RADIUS, headerFill,
                    new boolean[] { true, false, false, true });
            RenderUtils.drawRoundedRectAA(x, bodyTop, x + TAB_WIDTH, y + totalH,
                    CARD_RADIUS, bodyFill,
        ...
        // Faint border, and no lit top rim. At full strength the bottom edge
        // catches more light than the panel it sits on and reads as a raised
        // lip; light comes from above, so a bright bottom edge is backwards.
        int borderAlpha = Math.min(255, (int) (0x1C * (edgeAlpha / 255.0F)));
    ...
    private void drawActionRow(ButtonSetting button, int mouseX, int mouseY, CompactPalette palette,
        ...
        // A white wash rather than palette.card: the card colour is translucent
        // and would stack with the panel behind it into an opaque slab.
        int bg = ((hovered ? 0x2E : 0x12) << 24) | 0xFFFFFF;
    ...
    private void drawConfigRow(Config cfg, int mouseX, int mouseY, CompactPalette palette,
        ...
        int bg = current
                ? withAlpha(themeColor, hovered ? 0x76 : 0x60)
                : (((hovered ? 0x2A : 0x10) << 24) | 0xFFFFFF);
