...
public class SpotifySource implements MusicSource {
    ...
    private final ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "SpotifySource");
        t.setDaemon(true);
        return t;
    });
    ...
    @Override
    public boolean supportsControls() { return true; }
    ...
    private void poll() {
        ...
        try {
            ...
            int code = c.getResponseCode();
            if (code == 204) { currentTrack = null; status = "Nothing playing"; c.disconnect(); return; }
            if (code == 401) { currentTrack = null; status = "Token expired";   c.disconnect(); return; }
            if (code != 200) { status = "HTTP " + code;                          c.disconnect(); return; }
    ...
    private void parseTrack(String json) {
        try {
            ...
            if (!root.has("item") || root.get("item").isJsonNull()) {
                currentTrack = null; status = "Nothing playing"; return;
            }
            ...
            for (int i = 0; i < artistArr.size(); i++) {
                if (i > 0) sb.append(", ");
                sb.append(artistArr.get(i).getAsJsonObject().get("name").getAsString());
            }
            ...
            currentTrack = new MusicTrack(title, sb.toString(), artUrl, progMs, durMs, playing);
