...
public class RenderUtils {
    ...
    private static void setupSdfShader() {
        ...
        try {
            ...
            String vert =
                "#version 120\n" +
                // 0 on the quad's left edge, 1 on its right — the gradient
                // ramp. Interpolated by the rasteriser rather than recovered
                // from gl_FragCoord, so it needs nothing from the pixel-space
                // uniforms to be correct.
                "varying float vGx;\n" +
                "void main() {\n" +
                "  vGx = gl_MultiTexCoord0.x;\n" +
            ...
            String frag =
                ...
                // for a flat fill, so the mix below is a no-op there.
                ...
                "  vec4 col = mix(uColor, uColor2, clamp(vGx, 0.0, 1.0));\n" +
    ...
    /**
     * ponytail: PLACEHOLDER — renders {@code colorTop} flat, ignoring
     * {@code colorBottom}. TargetHUD already calls this two-colour overload but
     * it was never written, so the tree did not compile. A true vertical
     * gradient needs the SDF fragment shader and {@link #drawSdfShape} to carry
     * two colours and lerp on the local Y; do that when the gradient actually
     * matters. Unrelated to the Clutch fix — this only unblocks the build.
     */
    public static void drawRoundedRectAA(float x, float y, float x1, float y1, float radius, int colorTop, int colorBottom, boolean[] round) {
        drawRoundedRectAA(x, y, x1, y1, radius, colorTop, round);
    }
    ...
    public static void drawShadowedRoundedRect(float x, float y, float w, float h,
        ...
        // Same transform on the un-offset centre — the shader knocks the
        // caster's own silhouette out of the shadow.
        float pxClipC = pSx * (mvSx * (x + halfWGui) + mvTx) + pTx;
        float pyClipC = pSy * (mvSy * (y + halfHGui) + mvTy) + pTy;
        float panelCxPx = (pxClipC + 1.0F) * 0.5F * vpW + vpX;
        float panelCyPx = (pyClipC + 1.0F) * 0.5F * vpH + vpY;
        ...
        GL20.glUniform2f(shadowU_PanelCenter, panelCxPx, panelCyPx);
